...
MouseTool 会读取 macOS 当前的 Mission Control 快捷键配置；如果无法读取，则回退到系统默认的 `Fn + Control + Left` 和 `Fn + Control + Right`。
...
- `SpaceSwitcher.swift`：读取系统 Space 切换快捷键配置并发送快捷键。
...
请检查 Mission Control 的“向左移动一个空间”和“向右移动一个空间”快捷键是否已启用。MouseTool 会读取系统当前配置；如果刚刚修改过快捷键，重启 MouseTool 后会重新读取。
